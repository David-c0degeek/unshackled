...
export const BRIDGE_LOGIN_INSTRUCTION =
  'Remote Control is only available with claude.ai accounts. Please use `/login` to sign in with your claude.ai account.'

/** Full error printed when `unshackled remote-control` is run without auth. */
...
/**
 * How `unshackled remote-control` chooses session working directories.
...
export type BridgeConfig = {
   ...
   * instead of creating a new one. Used by `unshackled remote-control
