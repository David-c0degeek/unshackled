...
import { getUnshackledConfigHomeDir, isEnvTruthy } from './utils/envUtils.js'
...
/**
 * Unshackled parses history for pasted content references to match back to
...
async function* makeLogEntryReader(): AsyncGenerator<LogEntry> {
  ...
  const historyPath = join(getUnshackledConfigHomeDir(), 'history.jsonl')
...
export function addToHistory(command: HistoryEntry | string): void {
  // Skip history when running in a tmux session spawned by Unshackled's Tungsten tool.
