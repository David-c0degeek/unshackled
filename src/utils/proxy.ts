 ...
 *   env var is set by `unshackled ssh` on the remote CLI to route API calls through
...
export function getProxyFetchOptions(opts?: { forAnthropicAPI?: boolean }): {
  ...
  // ANTHROPIC_UNIX_SOCKET tunnels through the `unshackled ssh` auth proxy, which
