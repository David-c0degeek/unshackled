...
import { getUnshackledConfigHomeDir } from './envUtils.js'
...
function getSessionsDir(): string {
  return join(getUnshackledConfigHomeDir(), 'sessions')
}

/**
 * Kind override from env. Set by the spawner (`unshackled --bg`, daemon
...
/**
 * True when this REPL is running inside a `unshackled --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `unshackled ps` sees everything
...
export async function registerSession(): Promise<boolean> {
  ...
  try {
    ...
    // this, the PID file's sessionId goes stale and `unshackled ps` sparkline
...
/**
 * Push live activity state for `unshackled ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // See David-c0degeek/unshackled#34210.
    ...
    if (isProcessRunning(pid)) {
      ...
      // or UNSHACKLED_CONFIG_DIR), a Windows PID won't be probeable from WSL
