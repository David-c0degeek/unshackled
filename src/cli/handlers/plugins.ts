 ...
 * These are dynamically imported only when `unshackled plugin *` or `unshackled plugin marketplace *` runs.
 */
...
export async function pluginListHandler(options: {
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        'No plugins installed. Use `unshackled plugin install` to install a plugin.',
      )
    }
  }
