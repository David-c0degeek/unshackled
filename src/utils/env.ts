...
export const getGlobalClaudeFile = memoize((): string => {
  ...
  if (
    getFsImplementation().existsSync(
      join(getUnshackledConfigHomeDir(), '.config.json'),
    )
  ) {
    return join(getUnshackledConfigHomeDir(), '.config.json')
  }

  const filename = `.claude${fileSuffixForOauthConfig()}.json`
  return join(process.env.UNSHACKLED_CONFIG_DIR || homedir(), filename)
})
