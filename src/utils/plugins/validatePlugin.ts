 ...
 * one into the other. Surfaced as warnings by `unshackled plugin validate` since
...
export async function validatePluginManifest(
  ...
  // them. `unshackled plugin validate` is a developer tool — authors running it
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Unshackled ` +
  ...
  if (result.success) {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Unshackled accepts ` +
 ...
 * for the load path, but authors running `unshackled plugin validate` want a hard
