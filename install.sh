...
# unshackled installer
# Usage: curl -fsSL https://raw.githubusercontent.com/paoloanzn/unshackled/main/install.sh | bash
...
REPO="https://github.com/paoloanzn/unshackled.git"
INSTALL_DIR="$HOME/unshackled"
...
ART
  printf "${RESET}"
  printf "${DIM}  The free build of Unshackled${RESET}\n"
  echo ""
}
...
build_binary() {
  info "Building unshackled (all experimental features enabled)..."
  cd "$INSTALL_DIR"
...
link_binary() {
  ...
  ln -sf "$INSTALL_DIR/cli-dev" "$link_dir/unshackled"
  ok "Symlinked: $link_dir/unshackled"
...
printf "    ${CYAN}unshackled${RESET}                          # interactive REPL\n"
printf "    ${CYAN}unshackled -p \"your prompt\"${RESET}          # one-shot mode\n"
echo ""
printf "  ${BOLD}Set your API key:${RESET}\n"
printf "    ${CYAN}export ANTHROPIC_API_KEY=\"sk-ant-...\"${RESET}\n"
echo ""
printf "  ${BOLD}Or log in with Claude.ai:${RESET}\n"
printf "    ${CYAN}unshackled /login${RESET}\n"
echo ""
printf "  ${DIM}Source: $INSTALL_DIR${RESET}\n"
printf "  ${DIM}Binary: $INSTALL_DIR/cli-dev${RESET}\n"
printf "  ${DIM}Link:   ~/.local/bin/unshackled${RESET}\n"
echo ""
