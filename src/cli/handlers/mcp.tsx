 ...
 * These are dynamically imported only when the corresponding `unshackled mcp *` command runs.
 */
...
export async function mcpListHandler(): Promise<void> {
  ...
  if (Object.keys(configs).length === 0) {
    ...
    console.log('No MCP servers configured. Use `unshackled mcp add` to add a server.');
  } else {
    // biome-ignore lint/suspicious/noConsole:: intentional console output
...
export async function mcpResetChoicesHandler(): Promise<void> {
  ...
  cliOk('All project-scoped (.mcp.json) server approvals and rejections have been reset.\n' + 'You will be prompted for approval next time you start Unshackled.');
}
